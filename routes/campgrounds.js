...
//Create - Create new campgroud and add to database 
...
router.get('/:id', function(req, res){
    //find the campground with correct id
...
router.put("/:id", middleware.checkCampgroundOwnership, function(req, res){
    ...
    Campground.findByIdAndUpdate(req.params.id.trim(), req.body.campground, function(err, updatedCampground){
        if(err){
            res.redirect("/campgrounds")
            console.log(err)
        } else {
...
router.delete("/:id", middleware.checkCampgroundOwnership, function(req, res){
    Campground.findByIdAndRemove(req.params.id.trim(), function(err){
        if(err){
            res.redirect("/campgrounds");
        } else {
            res.redirect("/campgrounds");
